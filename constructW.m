...
%         fea = fea ./ repmat(max(1e-10,feaNorm),1,size(fea,2));
        ...
        if issparse(fea)
            ...
            feaNorm = sum(fea2.^2,1).^.5;
            for i = 1:nSmp
                fea2(:,i) = fea2(:,i) ./ max(1e-10,feaNorm(i));%归一化处理，求L2范数
            end
            fea = fea2';
            clear fea2;
        else
            feaNorm = sum(fea.^2,2).^.5;%求每一行的2范数
            for i = 1:nSmp
                fea(i,:) = fea(i,:) ./ max(1e-12,feaNorm(i));%归一化
            end
        end
    end
...
W = max(W,W');%将W与其转置取最大值，从而保证对称性。



elapse = cputime - tmp_T;%计算整个函数的执行时间，并将其存储在变量elapse中。
