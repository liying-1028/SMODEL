...
for dsi = 1:length(ds)
    ...
    [graph_K,S_K] = sparse_similarity_matrices_descend(inverse_distance,3);%12
    ...
    [LD_new,LD_new_gene] = processLD(LD_1, inverse_distance,3, 0.5);%knn
    [rows, cols] = size(LD_1);
        ...
        for iter=1:iters
            ...
            for lam = 1 : length(r1)
                ...
                for beta_index = 1:length(beta1)
                    ...
                    tic;
                    [A,W,alpha,obj,P] = Yvectest_res_1(X,Y,anchor(ichor),lambda,K,beta,avgS);
                    plot(obj);
                    xlabel('Iteration');
                    ylabel('Loss');
                    title('Loss Curve');
